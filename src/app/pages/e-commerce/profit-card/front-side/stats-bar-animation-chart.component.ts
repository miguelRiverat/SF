  ...
  ngAfterViewInit() {
    this.theme.getJsTheme()
      ...
      .subscribe(config => {
        ...
        this.getData(profitBarAnimationEchart)
        
    });
  }

  getData(profitBarAnimationEchart) {
    this.http
      .get(`${this.url}?type=month`)
        .subscribe(data => {
          let firstLine = []
          let secondLine = []
          data['rows'].forEach((current, index) => {
            firstLine.push(current.unidades)
            secondLine.push(current.pesos)
          })
